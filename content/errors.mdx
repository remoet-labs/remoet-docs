...
MCP returns JSON-RPC errors, and separates a key that is wrong from a key that is absent.
...
{ "jsonrpc": "2.0", "error": { "code": -32001, "message": "\"get_profile\" needs a Remoet account. search_jobs and get_listing work without one." }, "id": 1 }
```
